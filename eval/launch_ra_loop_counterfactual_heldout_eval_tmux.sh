...
# Six guarded workers for the frozen held-out CRA/NPC evaluation protocol.

PROJECT_ROOT=/home/imc/yzy/RA_LOOP
SESSION=ra_loop_counterfactual_heldout_eval
LAUNCHER="${PROJECT_ROOT}/eval/launch_ra_loop_counterfactual_heldout_eval.sh"
...
if [[ $# -eq 0 || "${1:-}" == --plan ]]; then
  ...
  echo "session=${SESSION} models=3 tasks=10 pairs/task=20 episodes=1200 GPUs=2..7"
...
for index in "${!GPUS[@]}"; do
  ...
  for job in "${jobs[@]}"; do
    ...
    pane_command+=" if [[ \${overall} -eq 0 ]]; then bash '${LAUNCHER}' --run '${source}' '${step}' '${task_id}' '${gpu}' 2>&1 | tee '${log_file}'; code=\${PIPESTATUS[0]}; echo '[HELDOUT_EVAL_EXIT]' '${source}' 'step${step}' 'task${task_id}' \${code} | tee -a '${log_file}'; if [[ \${code} -ne 0 ]]; then overall=\${code}; fi; fi;"
  done
  pane_command+=" echo '[HELDOUT_EVAL_WORKER_EXIT]' \${overall}; exec bash -i"
  if [[ $index -eq 0 ]]; then
