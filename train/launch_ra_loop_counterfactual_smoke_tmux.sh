...
SESSION=ra_loop_counterfactual_smoke
TRAINER="${PROJECT_ROOT}/train/ra_loop_counterfactual_smoke.sh"
LOG_ROOT="${PROJECT_ROOT}/logs/ra_loop_counterfactual_smoke_20260724"

if [[ $# -eq 0 || "${1:-}" == "--plan" ]]; then
  [[ $# -le 1 ]] || exit 2
  echo "Training is NOT started."
  echo "Final command: bash $0 --run <GPU_ID:2-7>"
  echo "tmux=${SESSION} profile=counterfactual_smoke steps=2 calibration=1 active_update=1"
