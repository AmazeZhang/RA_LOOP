...
# Start the bounded one-step stratified smoke in a persistent shared tmux.
# Safe default prints the plan; --run performs final checks and launches it.

PROJECT_ROOT=/home/imc/yzy/RA_LOOP
SESSION=ra_loop_stratified_connectivity_smoke
WINDOW=run1
TRAINER="${PROJECT_ROOT}/train/ra_loop_stratified_connectivity_smoke.sh"
OUTPUT_ROOT="${PROJECT_ROOT}/outputs/ra_loop_stratified_connectivity_smoke"
LOG_ROOT="${PROJECT_ROOT}/logs/ra_loop_stratified_connectivity_smoke_20260721"
...
if [[ $# -eq 0 || "${1:-}" == "--plan" ]]; then
  ...
  echo "Smoke is NOT started. Final command after confirmation:"
  echo "  bash $0 --run 7"
  echo "session=${SESSION} GPU=7 steps=1 K=8 tasks=1 horizon=220 expected=12-15m"
