...
if [[ $# -eq 0 || ( $# -eq 1 && "$1" == "--plan" ) ]]; then
  ...
  echo "lambda=0.25 physical_gpu=0 log=${LOG_DIR}/lambda025_gpu0.log"
...
lambda025_command="bash '${LAMBDA025_TRAINER}' --run 0 2>&1 | tee '${LOG_DIR}/lambda025_gpu0.log'; code=\${PIPESTATUS[0]}; echo '[RA_LOOP_EXIT]' \${code} | tee -a '${LOG_DIR}/lambda025_gpu0.log'; exec bash"

tmux new-session -d -s "${SESSION}" -n lambda0_gpu7 "${lambda0_command}"
tmux new-window -t "${SESSION}" -n lambda025_gpu0 "${lambda025_command}"

echo "Started ${SESSION}."
echo "Attach: tmux attach -t ${SESSION}"
echo "Windows: lambda0_gpu7, lambda025_gpu0"
